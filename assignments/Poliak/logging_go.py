...
class LoggingGeneticOptimization(Heuristic):
    ...
    def check_log(self):
        rat_current = self.neval / self.maxeval
        rat_log = (len(self.log_data) + 1) / (NUM_LOGS+1)
        if rat_log < rat_current:
            self.log({"neval" : self.neval, "y" : self.best_y, "x" : self.best_x})
